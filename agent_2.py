...
logger.debug("Agent 2 script started")
...
# Agent 2 specific vectorized database (keys stored in Redis)
agent_id = "agent_2"  # Each agent has its unique identifier
...
try:
        ...
        for message in pubsub.listen():
            if message['type'] == 'message':
                ...
                if "Forecast" in message['data'].decode('utf-8'):
                    ...
                    r.publish(results_channel, f"Agent 2: {forecast}")
    ...
    logger.debug("Agent 2 script completed successfully")
except Exception as e:
    pubsub.unsubscribe()
